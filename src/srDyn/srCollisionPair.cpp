...
typedef real dVector3[4];
typedef real dMatrix3[12];
...
inline real Inner (const real *a, const real *b)
{ return ((a)[0]*(b)[0] + (a)[1]*(b)[1] + (a)[2]*(b)[2]); }

inline real Inner14(const real *a, const real *b)
{ return ((a)[0]*(b)[0] + (a)[1]*(b)[4] + (a)[2]*(b)[8]); }

inline real Inner41(const real *a, const real *b)
{ return ((a)[0]*(b)[0] + (a)[4]*(b)[1] + (a)[8]*(b)[2]); }

inline real Inner44(const real *a, const real *b)
...
inline void dMULTIPLY0_331(real *A, const real *B, const real *C)
{ dMULTIPLYOP0_331(A,=,B,C) }

inline void dMULTIPLY1_331(real *A, const real *B, const real *C)
...
void cullPoints (int n, real p[], int m, int i0, int iret[])
{
...
static int intersectRectQuad (real h[2], real p[8], real ret[16])
{
...
{
	const real fudge_factor = 1.05;
	dVector3 p,pp,normalC;
	const real *normalR = 0;
	real A[3],B[3],R11,R12,R13,R21,R22,R23,R31,R32,R33,Q11,Q12,Q13,Q21,Q22,Q23,Q31,Q32,Q33,s,s2,l;
...
#undef TEMP_INNER14
	...
	real point[24];		// penetrating contact points
	real dep[8];			// depths for those points
	real det1 = dRecip(m11*m22 - m12*m21);
	m11 *= det1;
	m12 *= det1;
	m21 *= det1;
	m22 *= det1;
...
int	CollisionPair::_BoxSphere__________MARK8()
{
	...
	real r0 = 0.5 * pRightCol->GetDimension()[0];	// radius
							...
							dVector3 lret, dVector3 bret)
{
	...
	do {
		...
		if (next_dd2dt >= 0) {
			real m = (next_dd2dt-dd2dt)/(next_t - t);
			t -= dd2dt/m;
			goto got_answer;
		}
...
int dColDetBoxSphere__MARK7(real r0, const SE3 &T0,
...
int	CollisionPair::_BoxCapsule_________MARK8()
{
	...
	// Capsule
	real r0 = 0.5 * pRightCol->GetDimension()[0];	// radius
	real h0 = 0.5 * pRightCol->GetDimension()[1];	// half of height
	SE3 & T0 = pRightCol->GetFrame();
	...
	if (SquareSum(pl1-pl2) > SR_EPS ) //  line contact 2점을 return
	{
		...
		normal /= normal_sqr;
		real penetration = r0 - normal_sqr;
...
int CollisionPair::_SphereBox__________MARK8()
{
	// Sphere
	SE3 & T0 = pLeftCol->GetFrame();
	real r0 = 0.5 * pLeftCol->GetDimension()[0];	// radius
...
int CollisionPair::_SphereSphere_______MARK8()
{
	...
	real r0 = 0.5 * pLeftCol->GetDimension()[0];

	// Sphere2
	SE3 & c1 = pRightCol->GetFrame();
	real r1 = 0.5 * pRightCol->GetDimension()[0];
...
int dSphereSphere__MARK7(real r0, const Vec3 &c0,
						 real r1, const Vec3 &c1,
						 ContactConstraint * pContactConstraint)
{
	real rsum = r0 + r1;
...
int CollisionPair::_SphereCapsule______MARK8()
{
	...
	// Sphere
	SE3 & T1 = pLeftCol->GetFrame();
	real r1 = 0.5 * pLeftCol->GetDimension()[0];	// radius

	// Capsule
	SE3 & T0 = pRightCol->GetFrame();
	real r0 = 0.5 * pRightCol->GetDimension()[0];	// radius
	real h0 = 0.5 * pRightCol->GetDimension()[1];
...
int dColDetSphereBox__MARK7(real r0, const SE3 &T0,
						   const Vec3 &size, const SE3 &T1,
						   ContactConstraint * pConstraint)
{
	bool inside_box = true;
...
int CollisionPair::_CapsuleBox_________MARK8()
{
	// Capsule
	real r0 = 0.5 * pLeftCol->GetDimension()[0];
	real h0 = 0.5 * pLeftCol->GetDimension()[1];
	SE3 & T0 = pLeftCol->GetFrame();
...
int CollisionPair::_CapsuleCapsule_____MARK8()
{
	...
	real r1 = 0.5 * pRightCol->GetDimension()[0];
	real h1 = 0.5 * pRightCol->GetDimension()[1];
	SE3	& T1 = pRightCol->GetFrame();
	...
	Vec3 c = c1 - c0;
	real d0d1 = Inner(d0, d1);
	real cd0 = Inner(c, d0);
	real cd1 = Inner(c, d1);
	real D = 1.0 - d0d1 * d0d1;
	real t0, t1;
...
//	h1 *= 0.5;							// line segment 1 is c1 + t1 * d1, where t1 \in (-h1, h1).
	...
	{
		real tu = h0 * d0d1 - cd1;	// project c0 + h0 * d0 to line (c1, d1)
		real tl = -h0 * d0d1 - cd1;	// project c0 - h0 * d0 to line (c1, d1)
...
int CollisionPair::_PlaneSphere________MARK8()
{
	...
	Vec3 pt = c0 - c1;
	Vec3 normal(&T0[6]);
	real penetration = Inner(normal,pt) + r1;
	if ( penetration < 0.0 )
	{
		return 0;
	}
...
int CollisionPair::_PlaneCapsule_______MARK8()
{
	...
	real sign = (Inner(normal,d1) > 0.0) ? -1.0 : 1.0;
...
int CollisionPair::_SpherePlane________MARK8()
{
	...
	// Sphere
	SE3 & T1 = pLeftCol->GetFrame();
	real r1 = 0.5 * pLeftCol->GetDimension()[0];
...
int CollisionPair::_CapsulePlane_______MARK8()
{
	...
	SE3 & T1 = pLeftCol->GetFrame();
	real r1 = 0.5 * pLeftCol->GetDimension()[0];
	real h1 = 0.5 * pLeftCol->GetDimension()[1];
...
{
	real rsum = pLeftCol->GetBoundingRadius() + pRightCol->GetBoundingRadius();
