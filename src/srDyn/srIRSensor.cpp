...
real& srIRSensor::GetDetectedValue()
{
	return m_DetectedValue;
}

real srIRSensor::GetMaxRange()
{
	return m_MaxRange;
}

real srIRSensor::GetMinRange()
{
	return m_MinRange;
}

void srIRSensor::SetMaxRange(real max)
{
	if(max > m_MinRange) {
		m_MaxRange = max;
	}
...
void srIRSensor::SetMinRange(real min)
{
...
void srIRSensor::SetRange(real max, real min)
{
	if(max > min && min >= 0.0) {
		m_MaxRange = max;
		m_MinRange = min;
