...
bool	LCPSolver_SPARSE_PGS____MARK1(int n, int * r, int * c, int * rowofs, real * A,
									  real * x, real * b, real * lo, real * hi,
...
bool	LCPSolver_PGS____MARK3(int n, int nskip, int nub, real * A, real * x, real * b, 
							   real * lo, real * hi, int * findex, PGSOption * option)
{
	...
	int i, j, iter, idx;
	bool sentinel;
	real old_x, new_x, hi_tmp, lo_tmp, dummy, ea;
	real * A_ptr;
	real one_minus_sor_w = 1.0 - (option->sor_w);
...
bool	LCPSolver_PGS____MARK4(int n, int nskip, int /*nub*/, real * A, real * x, real * b, 
							   real * lo, real * hi, int * findex, PGSOption * option)
{
	// LDLT solver will work !!!
	//if (nub == n)
...
void ISLAND::_PGS_Solve_Constraint_____________________MARK7(PGSOption * option)
{
	...
	ALLOC_BK(real, lo, nt);			// lo : lower bound of lambda
	ALLOC_BK(real, hi, nt);			// hi : upper bound of lambda
	ALLOC_BK(int, findex, nt);			// findex : friction index
	ALLOC_BK(real, rhs,nt);			// rhs : c - J(v_old + invM*f_ext*h) where c is max( penet/h , -e*J*v_old )
	ALLOC_BK(real, lambda, nt);		// lambda : constraint force
	ALLOC_BK(real, invMc, j);			// ivMc : Constraint inverse mass matrix ( J*invM*Jt )
