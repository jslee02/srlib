...
real ClosedLoop::erp_closedloop = 0.8;
real ClosedLoop::allowederror = 0.01;
real ClosedLoop::maximum_erp_velocity = 10;

void ClosedLoop::SetErp(real _erp)
{
	erp_closedloop = _erp;
}

void ClosedLoop::SetAllowedPenetration(real _allowedpenetration)
{
	allowederror = _allowedpenetration;
}

void ClosedLoop::SetMaximumErpVelocity(real _maximum_erp_velocity)
{
	maximum_erp_velocity = _maximum_erp_velocity;
}

inline void ClosedLoop::GetError(real _recip_timestep)
{
...
void ClosedLoop::GetInformation(ConstraintInfo * info)
{
	...
	memcpy(info->_lambda,lambda,6*sizeof(real));
}
...
void ClosedLoop::GetDelVelocity(real * sjari)
{
...
void ClosedLoop::SetImpulse(real * _lambda)
{
	memcpy(lambda,_lambda,6*sizeof(real));
