...
real ContactConstraint::erp_contact = 0.8;
real ContactConstraint::allowedpenetration = 0.01;
real ContactConstraint::bouncing_threshold = 0.2;
real ContactConstraint::maximum_erp_velocity = 10;
real ContactConstraint::maximum_bouncing_velocity = 30;


void ContactConstraint::SetErp(real _erp)
{
	erp_contact = _erp;
}

void ContactConstraint::SetAllowedPenetration(real _allowedpenetration)
{
	allowedpenetration = _allowedpenetration;
}

void ContactConstraint::SetBouncingThreshold(real _bouncingthreshold)
{
	bouncing_threshold = _bouncingthreshold;
}

void ContactConstraint::SetMaximumErpVelocity(real _maximum_erp_velocity)
{
	maximum_erp_velocity = _maximum_erp_velocity;
}

void ContactConstraint::SetMaximumBouncingVelocity(real _maximum_bouncing_velocity)
{
...
real	ContactConstraint::_PRESTEP_Calculate_FrictionCoeff___________MARK7(real leftmat, real rightmat)
{
	return min(leftmat, rightmat);
}

real	ContactConstraint::_PRESTEP_Calculate_RestitutionCoeff________MARK7(real leftmat, real rightmat)
{
	//return min(leftmat, rightmat);
...
void			ContactConstraint::HowAboutThisPoint(Vec3& point , Vec3& normal , real & penetration)
{
...
inline int ContactConstraint::SortContactPoints(Vec3& localpoint_Left, real & penetration)
{
	...
	real area_n = SquareSum(Cross(a10,a20));
	real area_1 = SquareSum(Cross(an0,a20));
	real area_2 = SquareSum(Cross(an0,a10));
	real area_0 = SquareSum(Cross(a21,an1));
...
{
	real dist2 = CONTACT_THRESHOLD_SQR;
	real dist1;
...
inline void ContactConstraint::InsertContactPointAt(Vec3 & point, Vec3 & normal, real & penetration, Vec3& localpoint_Left, int insertIdx)
{
...
void ContactConstraint::GetInformation(ConstraintInfo * info)
{
	...
	if (bFriction)
	{
		...
		for (i = 0 ; i < nContactPts ; i++)
		{
			...
			real c = pContactPts[i]->Penetration - allowedpenetration;
			if (c < 0.0)
			{
				c = 0.0;
			}
			...
			if (bBounce)
			{
				...
				{
					real newc = rRestitution_coeff * neg_relvel;
...
inline void ContactConstraint::__getdelvelocity_Leftside(real *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_Rightside(real *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_Bothsides(real *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
inline void ContactConstraint::__getdelvelocity_None(real *sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
		sjari[i] = 0.0;
...
void ContactConstraint::_getdelvelocity_BothMove(real *sjari)
{
...
void ContactConstraint::_getdelvelocity_LeftMove(real *sjari)
{
	if (pLSystem->m_bExcited)
		__getdelvelocity_Leftside(sjari);
	else
...
void ContactConstraint::_getdelvelocity_RightMove(real *sjari)
{
	if (pRSystem->m_bExcited)
		__getdelvelocity_Rightside(sjari);
	else
...
void ContactConstraint::GetDelVelocity(real * sjari)
{
...
void ContactConstraint::_getrelvelocity_LeftMove(real * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::_getrelvelocity_RightMove(real * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::_getrelvelocity_BothMove(real * sjari)
{
	for (int i = 0 ; i < nd ; i++)
	{
...
void ContactConstraint::GetRelVelocity(real * sjari)
{
	(this->*m_pfn_getrelvelocity)(sjari);
}

void ContactConstraint::SetImpulse(real * _lambda)
{
	int i;
	if (bFriction)
	{
