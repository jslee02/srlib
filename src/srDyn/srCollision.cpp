...
real& srCollision::GetBoundingRadius()
{
...
//-- Ray Detection
bool srCollision::RayDetection(Vec3& point, Vec3& direction, real& range, real& dist)
{
...
//-- Touch Detection
bool srCollision::TouchDetection(Vec3& position, real radius, Vec3& point, Vec3& normal, real& penetration)
{
...
//-- Sonar Detection
bool srCollision::SonarDetection(SE3& T0, real & range , real & angle, bool & res)
{
...
bool RayToSphere(Vec3& point,Vec3& direction, real& range, 
				 const SE3 &T, real r,
				 real& dist)
{
	Vec3 q(&T[9]);
...
bool RayToCylinder(Vec3& point, Vec3& direction, real& range, 
				   const SE3 &T, real r, real h,
				   real& dist)
{
...
void Inv3by3(real* x, real* b)	// x * b = I
{
	real a11 = x[4]*x[8]-x[5]*x[7];
	real a12 = x[1]*x[8]-x[2]*x[7];
	real a13 = x[1]*x[5]-x[2]*x[4];
	real det = x[0]*a11-x[3]*a12+x[6]*a13;
	det = 1.0/det;
...
void Mult3by3(real* a, real* b, real* r)	// a*b=r, a,b,r: 3 by 3 matrix
{
...
void MultMV3by3(real*a, real*v, real*r)		// a*v=r, a: 3 by 3 matrix, v: 3-dim vector, r:3-dim vector
{
...
bool srCollision::_RayToBox(Vec3& point, Vec3& direction, real& range, real& dist)
{
	SE3 & T = GetFrame();
	...
	static real A[9];						//static RMatrix A(3,3);
	static real InvA[9];					//inverse matrix of A
	Vec3 q1(&T[0]),q2(&T[3]),q3(&T[6]);
	q1 *= size[0];
	q2 *= size[1];
	q3 *= size[2];

	memcpy(&A[0],&q1[0],sizeof(real)*3);	//memcpy(&A(0,0),&q1[0],sizeof(real)*3);
	memcpy(&A[3],&q2[0],sizeof(real)*3);	//memcpy(&A(0,1),&q2[0],sizeof(real)*3);
	memcpy(&A[6],&q3[0],sizeof(real)*3);	//memcpy(&A(0,2),&q3[0],sizeof(real)*3);

	static real B[3];						//static RMatrix B(3,1);
	memcpy(&B[0],&p1[0],sizeof(real)*3);	//memcpy(&B(0,0),&p1[0],sizeof(real)*3);
	...
	static real PC[3];						//static RMatrix C(3,1);
	static real C[3];						// InvA*PC=C;
	memcpy(&PC[0],&v[0],sizeof(real)*3);	//memcpy(&C(0,0),&v[0],sizeof(real)*3);

	Inv3by3(A, InvA);						//A = Inv(A);

	static real Z[3];						//static RMatrix Z(3,1);
											// InvA*B=Z

	MultMV3by3(InvA, B, Z);					//Z = A * B;
	MultMV3by3(InvA, PC, C);				//C = A * C;

	static real minVal, maxVal;
	real minGlobal = -10000, maxGlobal = 10000;
	for(int i=0;i<3;i++){
		if(Z[i] > 0){								//if(Z(i,0) > 0){
...
bool srCollision::_RayToSphere(Vec3& point, Vec3& direction, real& range, real& dist)
{
	SE3 & T = GetFrame();
	real r = 0.5*GetDimension()[0];
	//
...
bool srCollision::_RayToCylinder(Vec3& point, Vec3& direction, real& range, real& dist)
{
	SE3 & T = GetFrame();
	real r = 0.5*GetDimension()[0];
	real h = GetDimension()[1];
	//
...
bool srCollision::_RayToCapsule(Vec3& point, Vec3& direction, real& range, real& dist)
{
	SE3 & T = GetFrame();
	real r = 0.5*GetDimension()[0];
	real h = GetDimension()[1];
...
bool srCollision::_RayToPlane(Vec3& point, Vec3& direction, real& range, real& dist)
{
	SE3 & T = GetFrame();
	//
...
bool srCollision::_RayToUser(Vec3& /*point*/, Vec3& /*direction*/, real& /*range*/, real& /*dist*/)
{
	return false;
}
...
bool srCollision::_TouchToBox(Vec3& c0, real r0, Vec3& point, Vec3& normal, real& penetration)
{
	// Sphere
	...
	if ( inside_box )
	{
		...
		real min = size[0] - fabs(p[0]);
		real tmin = size[1] - fabs(p[1]);
		int idx = 0;
	...
	point = T1 * p;
	normal = c0 - point;
	real mag = sqrt(SquareSum(normal));
	penetration = r0 - mag;
...
bool srCollision::_TouchToSphere(Vec3& c0, real r0, Vec3& point, Vec3& normal, real& penetration)
{
	// Sphere1

	// Sphere2
	SE3 & c1 = GetFrame();
	real r1 = 0.5*GetDimension()[0];

	real rsum = r0 + r1;
	normal.SetValues(c0[0] - c1[9], c0[1] - c1[10], c0[2] - c1[11]);
	real normal_sqr = SquareSum(normal);
...
bool srCollision::_TouchToCylinder(Vec3& /*position*/, real /*radius*/, Vec3& /*point*/, Vec3& /*normal*/, real& /*penetration*/)
{
	// 미구현
	return false;
}

bool srCollision::_TouchToCapsule(Vec3& c1, real r1, Vec3& point, Vec3& normal, real& penetration)
{
	...
	// Capsule
	SE3 & T0 = GetFrame();
	real r0 = 0.5*GetDimension()[0];
	real h0 = GetDimension()[1];


	Vec3 dir(&T0[6]);
	Vec3 c0(&T0[9]);

	real t = Inner(dir, c1 - c0);
...
bool srCollision::_TouchToPlane(Vec3& c1, real r1, Vec3& point, Vec3& normal, real& penetration)
{
	// Plane
	SE3 & T0 = GetFrame();
...
bool srCollision::_TouchToUser(Vec3& /*position*/, real /*radius*/, Vec3& /*point*/, Vec3& /*normal*/, real& /*penetration*/)
{
	return false;
}
...
========================================================================== */
bool _ConeToSlab(Vec3& normal, Vec3& center, real min, real max, SE3& T0, real& range, real& radius)
{
	Vec3 Q;
	Vec3 m;
	...
	real A = Inner(normal, P);
	// project Q onto normal
	real B = Inner(normal, Q);
...
bool srCollision::_SonarToBox(SE3& T0, real & range , real & angle, bool & res)
{
	// box information
	SE3& Tbox = GetFrame();
	Vec3 dim = 0.5*GetDimension();
	real radius = range * tan(angle * 0.5);
...
bool srCollision::_SonarToSphere(SE3& T0, real & range , real & angle, bool & res)
{
	// Sphere
	SE3 & c = GetFrame();
	real r = 0.5*GetDimension()[0];
...
//		ASSERT(abs(intercept[1]) < 10e-6 && abs(intercept[2]) < 10e-6);
		if(intercept[0] <= 0.0) {	// intercept is above the apex
			...
			real s = intercept[0] * sin(angle * 0.5);
...
bool srCollision::_SonarToCapsule(SE3& /*T0*/, real & /*range*/, real & /*angle*/, bool & /*res*/)
{
	return false;
}

bool srCollision::_SonarToCylinder(SE3& /*T0*/, real & /*range*/, real & /*angle*/, bool & /*res*/)
{
	return false;
}

bool srCollision::_SonarToPlane(SE3& T0, real & range , real & angle, bool & res)
{
	// Plane
	SE3 & T1 = GetFrame();
...
bool srCollision::_SonarToUser(SE3& /*T0*/, real & /*range*/, real & /*angle*/, bool & /*res*/)
{
	return false;
}
