...
: srSensor()
{
	...
	m_NumSpots = (int)((real)m_Spread / m_Resolution) + 1;

	m_SpreadRad = DEG2RAD((real)m_Spread * 0.5);
...
real srRangeFinder::GetMaxRange()
{
	return m_MaxRange;
}

real srRangeFinder::GetMinRange()
{
	return m_MinRange;
}

void srRangeFinder::SetMaxRange(real max)
{
	if(max > m_MinRange) {
		m_MaxRange = max;
	}
...
void srRangeFinder::SetMinRange(real min)
{
...
void srRangeFinder::SetRange(real max, real min)
{
	if(max > min && min >= 0.0) {
		m_MaxRange = max;
		m_MinRange = min;
...
real srRangeFinder::GetResolution()
{
	return m_Resolution;
}
void srRangeFinder::SetResolution(real r)
{
	if(0.0 < r && r < m_Spread)
	{
		m_Resolution = r;
...
real* srRangeFinder::GetDetectedValue()
{
